...
### Audio Noise Remover and Equalizer v1
Lesson #80 combines tonal noise removal, a three-band equalizer, WAV processing, automated tests, and independent C audio metrics into one reproducible portfolio project.
#### Goal
Process a mono int16 PCM WAV file, suppress a selected tonal interference frequency, adjust three frequency bands, and save a new WAV file with numerical evidence and reproducible audio examples.
#### Processing
* Read mono int16 PCM audio and convert samples to float64 using division by 32768.0.
* Design a second-order IIR notch filter with iirnotch, convert its coefficients to SOS, and apply sosfilt.
* Apply a three-band FFT equalizer to the filtered signal.
* Convert gain settings from decibels to amplitude multipliers using 10 ** (gain_db / 20).
* Preserve the sample count and sampling frequency.
* If the output peak exceeds 0.99, reduce all samples by the same factor before writing.
* Leave quieter signals and silence at their existing level.
* Round and convert samples to int16 for WAV output.
* Reject an existing output path instead of overwriting it.
* Report the sampling frequency, sample count, applied output scaling factor, and output path.
The EQ bands are:
* Low: frequencies below 300 Hz.
* Mid: frequencies from 300 Hz up to, but excluding, 3000 Hz.
* High: frequencies at or above 3000 Hz.
#### Command-Line Interface
Arguments:
* input_path: mono int16 PCM WAV input.
* output_path: a new output file path.
* --noise-frequency: frequency in Hz, default 50.0; must be finite, positive, and below Nyquist.
* --quality-factor: finite positive notch quality factor, default 30.0.
* --low-gain-db: low-band gain, default 0.0 dB.
* --mid-gain-db: mid-band gain, default 0.0 dB.
* --high-gain-db: high-band gain, default 0.0 dB.
* All EQ gains must be finite and between -24 and +24 dB, inclusive.
Example from the repository root, using an existing input.wav and a new output.wav:
    python applications/signal_visualizer/audio_processor.py input.wav output.wav --noise-frequency 50 --quality-factor 30 --low-gain-db -6 --mid-gain-db 0 --high-gain-db 6
#### Reproducible Audio Demo
Python dependencies: NumPy and SciPy. Automated tests also require pytest.
Generate the audio examples from the repository root:
    python applications/signal_visualizer/audio_processor_demo.py
The demo generates five seconds of mono audio at 44100 Hz, containing 220500 samples:
* Useful tones: 200, 1000, and 4000 Hz, each with amplitude 0.2 before WAV quantization.
* Tonal interference: 50 Hz with amplitude 0.15 before WAV quantization.
* Notch settings: 50 Hz and Q = 30.
* Equalized output gains: -6, 0, and +6 dB.
Generated files in applications/signal_visualizer/audio_processor_demo/:
* noisy.wav: useful tones plus interference.
* cleaned.wav: notch processing with neutral EQ.
* equalized.wav: notch processing with the selected EQ gains.
Both processed outputs are generated by invoking the actual CLI on noisy.wav. Their reported output scaling factors were 1.0.
The demo refuses to overwrite existing demo WAV files. To regenerate them, first move or remove only those three generated WAV files.
#### Measured Audio Results
Measurements use the saved and reloaded WAV files. The interval from 2.0 to 5.0 seconds is used for spectral measurements to exclude the initial notch transient.
| Frequency (Hz) | Noisy amplitude | Cleaned amplitude | Equalized amplitude |
|---|---|---|---|
| 50 | 0.15000018 | 0.00000072 | 0.00000051 |
| 200 | 0.20000055 | 0.19999245 | 0.10023303 |
| 1000 | 0.19999964 | 0.19999937 | 0.20000060 |
| 4000 | 0.20000034 | 0.20000006 | 0.39905297 |
Measured EQ amplitude ratios relative to cleaned.wav:
| Frequency (Hz) | Measured gain | Expected gain |
|---|---|---|
| 200 | 0.501184 | 0.501187 |
| 1000 | 1.000006 | 1.000000 |
| 4000 | 1.995264 | 1.995262 |
The demo passed its noise attenuation, useful-tone preservation, and EQ gain checks.
#### Automated Tests
Run from the repository root:
    python -m pytest applications/signal_visualizer/test_audio_processor.py -q
Observed result: 8 passed in 6.59s.
Coverage includes:
* Neutral EQ with an odd sample count and unchanged input.
* Requested gains applied independently to the three bands.
* Notch attenuation and preservation of useful-tone amplitudes.
* Four WAV round-trip cases: silence, quiet audio, a peak just above 0.99, and an over-range signal.
* Output overwrite protection.
* CLI processing and rejection of an out-of-range EQ gain.
The WAV round-trip tolerance is half an int16 quantization step, 0.5 / 32768.0, plus a small numerical allowance.
#### Independent C Metrics
audio_metrics.c reads numeric samples from standard input and computes:
* Sample count.
* RMS amplitude: sqrt(sum_of_squares / sample_count).
* Peak absolute amplitude.
The C program accumulates these metrics without storing the full sample sequence. The Python validation script reads each saved WAV, sends its normalized samples to C as text, and compares the reported metrics with NumPy calculations.
After generating the demo files, compile and validate from the repository root on Windows with GCC:
    gcc -std=c11 -O2 -Wall -Wextra applications/signal_visualizer/audio_metrics.c -o applications/signal_visualizer/audio_processor_demo/audio_metrics.exe -lm
    python applications/signal_visualizer/audio_metrics_validation.py
The compilation completed without reported compiler warnings. All three WAV files contained 220500 samples.
| File | RMS, C and Python to displayed precision | Peak, C and Python to displayed precision |
|---|---|---|
| noisy.wav | 0.266927204592 | 0.721099853516 |
| cleaned.wav | 0.245383671434 | 0.717315673828 |
| equalized.wav | 0.323572140508 | 0.745117187500 |
Maximum observed absolute differences:
* RMS: 2.787e-13.
* Peak: 3.750e-13.
Both satisfy the 1e-10 validation tolerance. Empty input, invalid text, and NaN were also confirmed to produce a C failure exit code.
#### Scope and Limitations
* Processing is offline and loads the complete recording into memory.
* Input support is limited to nonempty mono int16 PCM WAV files.
* Noise removal targets a manually selected tonal frequency; it is not general broadband denoising or automatic noise detection.
* Useful content near the notch frequency may also be attenuated.
* Causal notch filtering introduces phase changes and an initial transient.
* EQ uses a whole-file FFT with abrupt band boundaries and can produce ringing or boundary artifacts.
* WAV quantization introduces small amplitude errors.
* RMS and peak describe signal level; they do not independently prove noise removal or perceived audio quality.
* The C utility is validated on short normalized audio sequences, not arbitrary-length or extreme-magnitude numeric input.
* Generated WAV files and the compiled executable are ignored by Git. The demo and validation source code are included for reproducibility.
#### Files
    applications/signal_visualizer/audio_processor.py
    applications/signal_visualizer/test_audio_processor.py
